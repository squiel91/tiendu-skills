...
- `currency` is the product's currency code (`'UYU'` or `'USD'`). It matches the store-level `store.currency.code` for single-currency stores but belongs to the product object so multi-currency catalogs and client-side JS can format product-specific prices correctly.
- Client-side price formatting (e.g. `Intl.NumberFormat`) must read `product.currency` when available; the Liquid `money` filter already resolves the currency from `store.currency.code` or an explicit argument.
